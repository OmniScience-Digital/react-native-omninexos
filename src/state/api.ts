...
const NETWORK_PATTERNS = [
  ...
  // Amplify/AWS SDK-specific wording seen when the device is offline
  // before a request even reaches the GraphQL endpoint (e.g. Cognito
  // Identity Pool credentials can't be fetched):
  "credentials",
  "getaddrinfo",
  "unable to connect",
  "no internet",
...
function netAwareError(e: any): { error: any } {
  // Amplify's client.graphql() doesn't always reject with a plain Error.
  // When the request never makes it out (device offline, credentials
  // fetch fails, request aborted mid-flight) it often rejects with an
  // object shaped like `{ errors: [...] }` and no top-level `.message`.
  // Falling straight to `String(e)` on that gives "[object Object]",
  // which matches none of NETWORK_PATTERNS, so the error was wrongly
  // classified as "real" instead of offline.
  const msg: string =
    e?.message ??
    e?.errors?.[0]?.message ??
    e?.error?.message ??
  ...
  if (isNetworkErr(msg) || e?.name === "AbortError") {
    return { error: { status: "FETCH_ERROR", error: msg } };
  }
  // Always return an object (never a bare string) so downstream
  // middleware can rely on a consistent `{ error, message }`-style shape
  // instead of falling back to RTK's generic "Rejected" default.
